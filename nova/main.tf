...
resource "juju_machine" "nova_compute_machines" {
  count  = var.placement.compute == null ? var.units.compute : 0
  model  = var.model
  series = var.series
  name   = format("%s%s", "nova_compute", count.index)
  constraints = "mem=4G"
}
...
resource "juju_application" "nova_compute" {
    ...
    placement = var.placement.compute == null ? join(",", [for machine in juju_machine.nova_compute_machines : split(":", machine.id)[1]]) : var.placement.compute
    lifecycle {
        ignore_changes = [ placement, ]
    }
}

resource "juju_machine" "nova_controller_machines" {
  count  = var.placement.cloud_controller == null ? var.units.cloud_controller : 0
  model  = var.model
  series = var.series
  name   = format("%s%s", "nova_controller", count.index)
  constraints = "mem=2G"
}
...
resource "juju_application" "nova_cloud_controller" {
    ...
    placement = var.placement.cloud_controller == null ? join(",", [for machine in juju_machine.nova_controller_machines : split(":", machine.id)[1]]) : var.placement.cloud_controller
    lifecycle {
        ignore_changes = [ placement, ]
    }
}
